 ...
 * Usage: DATABASE_URL=mysql://... npx tsx sync-news.ts
 */
...
async function main() {
  ...
  for (let i = 0; i < NEWS_TEMPLATES.length; i++) {
    ...
    await prisma.aiNews.upsert({
      ...
      update: {
        ...
        keywords: JSON.stringify([t.category, "AI", "人工智能", t.source.replace(" Blog", "").toLowerCase()]),
  ...
  console.log(`✅ Synced ${NEWS_TEMPLATES.length} news items to MySQL`);

  const cats: Record<string, number> = {};
  NEWS_TEMPLATES.forEach((n) => (cats[n.category] = (cats[n.category] || 0) + 1));
  console.log("\n📊 Categories:");
